    ...
    def run(self):
        ...
        for t in range(self.max_time_steps):
            ...
            if self.animate and (self.animate_from is None or len(self.episode_rewards) >= self.animate_from):
                self.animator.next(np.amax(observation * np.reshape(np.arange(observation.shape[2]) + 1, (1, 1, -1)), 2) + 0.5)
            ...
            if self.gif_saver is not None and \
                ...
                self.gif_saver.add(np.amax(observation * np.reshape(np.arange(observation.shape[2]) + 1, (1, 1, -1)), 2) + 0.5)
            ...
            if done:
                ...
                if self.animate and (self.animate_from is None or len(self.episode_rewards) >= self.animate_from):
                    self.animator.next(np.amax(new_observation * np.reshape(np.arange(observation.shape[2]) + 1, (1, 1, -1)), 2) + 0.5)
                ...
                if self.gif_saver is not None and \
                    ...
                    self.gif_saver.add(np.amax(new_observation * np.reshape(np.arange(observation.shape[2]) + 1, (1, 1, -1)), 2) + 0.5)
                ...
                self.env.reset()
                new_observation = self.env.state()
