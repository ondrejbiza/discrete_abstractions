...
def plot_epoch_losses(train_losses, valid_losses, train_indices, valid_indices, train_labels, valid_labels, epoch_size,
                    validation_freq, saver=None, save_name=None, ylim=None):

    fig = plt.figure()
    ax = fig.gca()

    for idx, label in zip(train_indices, train_labels):
        ax.plot(list(range(1, len(train_losses) + 1)), train_losses[:, idx], label=label)

    for idx, label in zip(valid_indices, valid_labels):
        ax.plot(
            (np.linspace(0, len(valid_losses), num=len(valid_losses)) * validation_freq) / epoch_size,
            valid_losses[:, idx], label=label, linestyle="dashed"
        )
...
def plot_all_losses(train_losses, valid_losses, train_indices, valid_indices, train_labels, valid_labels,
                      validation_freq, saver=None, save_name=None, ylim=None, log_scale=True):
    fig = plt.figure()
    ax = fig.gca()
    ...
    for idx, label in zip(valid_indices, valid_labels):
        ax.plot(
            (np.linspace(0, len(valid_losses), num=len(valid_losses)) * validation_freq),
            valid_losses[:, idx], label=label, linestyle="dashed"
        )
...
def plot_many_histograms(data_list, names_list, xlabel=None, num_bins=False, saver=None):
...
def plot_soft_histogram(data, name, saver, xlabel=None):

    weights = np.sum(data, axis=0)
    vals = np.array(list(range(data.shape[1])))

    fig = plt.figure()
    ax = fig.gca()
    ax.hist(vals, weights=weights, bins=len(vals))
...
def plot_5x5_grid(images, name, saver, cmap=None, norm=None):
    ...
    for i in range(25):
        ...
        axis.imshow(images[i], vmin=vmin, vmax=vmax, cmap=cmap, norm=norm)
