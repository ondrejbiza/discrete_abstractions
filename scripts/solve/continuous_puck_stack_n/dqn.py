...
from agents.solver_2 import Solver
from abstract_models.bisim.puck_stack_2 import BisimPuckStack as BisimPuckStack2
from abstract_models.bisim.puck_stack_3 import BisimPuckStack as BisimPuckStack3
...
def collect_factory_bisim(goal):

    if goal == 2:
        bisim_model = BisimPuckStack2
    else:
        bisim_model = BisimPuckStack3

    def collect_pre(env, action):
        return bisim_model.get_state_block(env.hand, env.layers, env.action_grid)
...
def main(args):
    ...
    env = ContinuousPuckStack(
        args.num_blocks, args.num_pucks, args.num_pucks, args.num_actions, height_noise_std=args.height_noise_std,
        random_shape=args.random_shape, action_failure_prob=args.action_failure_prob
    )
    ...
    agent = DQN(
        ...
        args.final_epsilon, args.max_time_steps, buffer_size=args.buffer_size,
        prioritized_replay=not args.disable_prioritized_replay, target_net=not args.disable_target_network,
        target_update_freq=args.target_update_freq, target_size=args.target_size, fix_dones=args.fix_dones
    )
    ...
    solver = Solver(
        ...
        collect_pre=t_collect_pre, collect_post=t_collect_post, collect_data=t_collect_data
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser("Solve stacking 2 pucks with the DQN agent.")
    ...
    parser.add_argument("--random-shape", default=False, action="store_true", help="add objects of random shapes")
    ...
    parser.add_argument("--fix-dones", default=False, action="store_true", help="fix dones for episode time limit")
