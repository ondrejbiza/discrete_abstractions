...
import random, numpy, argparse, logging, os, pickle
...
def dqn(env, replay_off, target_off, output_file_name, store_intermediate_result=False, load_path=None,
    ...
    while t < NUM_FRAMES:
        ...
        while (not is_terminated) and t < NUM_FRAMES:
            # Generate data
            #new_image = numpy.amax(env.state() * numpy.reshape(numpy.arange(7) + 1, (1, 1, -1)), 2) + 0.5
...
def main():
    ...
    policy_net = dqn(env, args.replayoff, args.targetoff, file_name, args.save, load_file_path, args.alpha)
    ...
    if args.save_dataset:
        ...
        for i in range(num_steps):
            ...
            t = Transition(
                s, int(action.cpu().numpy()[0, 0]), float(reward), s_prime, False, bool(terminated),
                q_values=q_values.cpu().numpy(), next_q_values=q_values_prime.cpu().numpy()
            )
